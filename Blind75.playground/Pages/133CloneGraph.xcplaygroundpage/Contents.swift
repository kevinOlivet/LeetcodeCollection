...
// This Node doesn't conform to Hashable nor Equatable etc but the Leetcode provided one does in the background so it passes
public class Node {
    public var val: Int
...
func cloneGraph(_ node: Node?) -> Node? {
    ...
    func dfs(node: Node) -> Node {
        if oldToNew.keys.contains(node) {
            return oldToNew[node]!
        }
        var copy = Node(node.val)
        oldToNew[node] = copy
        for nei in node.neighbors {
            let newNeighbor = dfs(node: nei!)
